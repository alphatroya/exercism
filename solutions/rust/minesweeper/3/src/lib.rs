pub fn annotate(minefield: &[&str]) -> Vec<String> {
    ...
    for (line, value) in minefield.iter().enumerate() {
        for (row, character) in value.chars().enumerate() {
            if character == '*' {
                increment_neighbors(&mut result, line, row)
            }
        }
    }
    result
...
fn increment_neighbors(result: &mut Vec<String>, line: usize, row: usize) {
    let row = row as isize;
    if line > 0 {
        if let Some(previous_line) = result.get(line - 1) {
            result[line - 1] = increment(&previous_line, vec![row - 1, row, row + 1])
        }
    }
    if let Some(current_line) = result.get(line) {
        result[line] = increment(&current_line, vec![row - 1, row, row + 1])
    }
    if let Some(next_line) = result.get(line + 1) {
        result[line + 1] = increment(&next_line, vec![row - 1, row, row + 1])
    }
}
