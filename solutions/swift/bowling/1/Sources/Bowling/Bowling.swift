final class Bowling {
    ...
    enum State {
        ...
        case strikeFirst(isStrike: Bool)
        case strikeSecond(isStrike: Bool, first: Int)
        case tenthThrowSpare
        case end
    }
    ...
    func roll(pins: Int) throws {
        ...
        case let .first(strikeCount, isSpare):
            ...
            if pins == 10 {
                turnNumber += 1
                if turnNumber == 10 {
                    state = .strikeFirst(isStrike: strikeCount > 0)
                } else {
        ...
        case let .strikeFirst(isStrike):
            scoreValue += isStrike ? 2 * pins : pins
            state = .strikeSecond(isStrike: pins == 10, first: pins)
        case let .strikeSecond(isStrike, first):
